    ...
    if main.status_code == 200:
        ...
        for item_id in content:
            if (item_id['quota_sqkm'])is not None:
                ...
                print(f'Remaining Quota in SqKm: {leftquota}')
                return(leftquota)
            else:
                print('No Quota Allocated')

    else:
        print(f'Failed with exception code: {str(main.status_code)}')
...
def get_thumbs_from_search_json(search_json):
    ...
    for i in range(len(tb)):
        ...
        filestring = f"{str(image_ids[i])}.jpg"
...
def construct_requestv2(name,aoi,item_ids):
    return {
        ...
        "tools": [{"clip": {"aoi": {"type": "Polygon", "coordinates": aoi}}}],
    }
...
if __name__ == "__main__":
    ...
    dfwdd_rows = []
    tflist = []
    for gtodcounter, g in enumerate(geojson_list):
